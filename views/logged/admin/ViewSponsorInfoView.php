...
$category = $sponsorsControler->getSponsorById($id);
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $_POST['action'] === 'archive' && $category["status"] === "Active") {
    $id = intval($_POST['id']);
    $sponsorsControler->archiveSponsor($id);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $_POST['action'] === 'restore' && $category["status"] === "Archived") {
    $id = intval($_POST['id']);
    $sponsorsControler->restoreSponsor($id);
    $currentUrl = $_SERVER['PHP_SELF'];
    $currentUrl .= "?id=" . $_GET['id'];
    header("Location: " . $currentUrl);
...
<body>
    ...
    <div class="row vh-100 bg-light rounded align-items-center justify-content-center mx-0 shadow-lg">
        <div class="col-md-6 text-center p-4 bg-white rounded shadow">
            <div class="mb-4">
                <h1 class="display-5 fw-bold text-primary">Sponsor Details</h1>
                <p class="text-muted">Here are the details of the selected sponsor:</p>
            </div>
    ...
    <img style="height:200px;width:200px;" src="../../inc/logged/img/sponsors/<?php echo $category["photo"]; ?>">
                </h4>
            ...
            <a href="index.php?action=modifysponsor&id=<?php echo $category["id"];?>" class="btn btn-primary mt-4 px-4 py-2 rounded-pill shadow-sm">Modify</a>
...
<?php } ?>
            <a href="sponsors.php" class="btn btn-primary mt-4 px-4 py-2 rounded-pill shadow-sm">Back to Sponsors</a>
