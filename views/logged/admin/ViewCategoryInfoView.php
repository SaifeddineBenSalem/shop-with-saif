...
if ($role != "admin")
    header("Location: ../index.php");
$categoryControler = new CategoriesControler();
$category = $categoryControler->getCategoryById($id);
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $_POST['action'] === 'archive' && $category["status"] === "Active") {
    $id = intval($_POST['id']);
    $categoryControler->archiveCategory($id);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $_POST['action'] === 'restore' && $category["status"] === "Archived") {
    $id = intval($_POST['id']);
    $categoryControler->restoreCategory($id);
    $currentUrl = $_SERVER['PHP_SELF'];
    $currentUrl .= "?id=" . $_GET['id'];
    header("Location: " . $currentUrl);
}
...
<body>
    ...
    <div class="row vh-100 bg-light rounded align-items-center justify-content-center mx-0 shadow-lg">
        <div class="col-md-6 text-center p-4 bg-white rounded shadow">
            <div class="mb-4">
                <h1 class="display-5 fw-bold text-primary">Category Details</h1>
                <p class="text-muted">Here are the details of the selected category:</p>
            </div>
            ...
            <h4 class="mb-3">
                ...
                <span class="text-secondary fw-bold">Last modified time:</span>
        ...
        <?php }?>
            ...
            <a href="index.php?action=modifycategory&id=<?php echo $category["id"];?>" class="btn btn-primary mt-4 px-4 py-2 rounded-pill shadow-sm">Modify</a>
...
<?php } ?>
            ...
            <a href="categories.php" class="btn btn-primary mt-4 px-4 py-2 rounded-pill shadow-sm">Back to Categories</a>
